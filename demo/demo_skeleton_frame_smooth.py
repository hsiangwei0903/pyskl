...
def parse_args():
    ...
    parser.add_argument(
        '--config',
        default='configs/posec3d/slowonly_r50_346_etri_50/joint.py',
    ...
    parser.add_argument(
        '--label-map',
        default='tools/data/label_map/etri.txt',
        help='label map file')
    parser.add_argument(
    ...
    parser.add_argument(
        '--clip-length', type=int, default=50, help='length of clip for continuous action recognition')
    parser.add_argument(
        '--vote-length', type=int, default=10, help='length for action recognition majority vote window')
...
def main():
    ...
    # basename = args.video.split('/')[-1]
    # data_root = args.video.replace(basename,'')

    # with open(osp.join(data_root,'gt/gt_kepts.pkl'),'r') as f:
    #     kepts = f.read()

    # gt_kepts = json.loads(kepts)
    # pose_results = []

    # for frame_id in range(num_frame):
    #     frame_info = []
    #     if frame_id in gt_kepts:
    #         frame_results = gt_kepts[frame_id]
    #         for trk_info in frame_results:
    #             trk_dict = {
    #                 'bbox':np.array(trk_info['bbox'],dtype=np.float32),
    #                 'keypoints':np.array(trk_info['keypoints'],dtype=np.float32)
    #             }
    #             frame_info.append(trk_dict)
    #     pose_results.append(frame_info)

    assert len(pose_results) == num_frame

    num_person = max([len(x) for x in pose_results])
    # Current PoseC3D models are trained on COCO-keypoints (17 keypoints)
    num_keypoint = 17

    action_labels = defaultdict()

    for i, _ in enumerate(tqdm(pose_results)):

        keypoint = np.zeros((num_person, clip_length, num_keypoint, 2),
                            dtype=np.float16)
        keypoint_score = np.zeros((num_person, clip_length, num_keypoint),
                                    dtype=np.float16)
        
        start = max(0,i-clip_length//2)
        end = min(i+clip_length//2-1,num_frame-1)
        ranges = [f for f in range(start,end+1)]
        diff = clip_length - len(ranges)
        if diff%2==0:
            ranges = [ranges[0]]*(diff//2) + ranges + [ranges[-1]]*(diff//2)
        else:
            ranges = [ranges[0]]*(diff//2+1) + ranges + [ranges[-1]]*(diff//2)
        
        assert len(ranges) == clip_length, print(start,end,diff,len(ranges),clip_length)
        
        for idx,ii in enumerate(ranges):
            poses = pose_results[ii]
            for j, pose in enumerate(poses):
                pose = pose['keypoints']
                keypoint[j, idx] = pose[:, :2]
                keypoint_score[j, idx] = pose[:, 2]
        ...
        action_labels[i] = label_map[results[0][0]]
    ...
    window = deque(maxlen=args.vote_length)

    # Initialize result defaultdict
    result = defaultdict(str)

    # Iterate over the frames
    for frame_id, prediction in action_labels.items():
        # Add the current frame to the window
        window.append(prediction)

        # Check if the window is filled
        if len(window) == n:
            # Calculate the majority vote using the mode function
            majority_vote = Counter(window).most_common()[0][0]

            # Update the result defaultdict with the majority vote for the middle frame in the window
            result[frame_id - n//2] = majority_vote

    action_labels = result

    for f_id,frame in enumerate(vis_frames):
        cv2.putText(frame, action_labels[f_id], (10, 30), FONTFACE, FONTSCALE,
